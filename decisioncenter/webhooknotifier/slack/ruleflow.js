...
const EVENT_RULEFLOW_CREATED = "RuleflowCreated";
const EVENT_RULEFLOW_UPDATED = "RuleflowUpdated";
const EVENT_RULEFLOW_DELETED = "RuleflowDeleted";
...
const slackPretext = (event, showEmoji) => {
    ...
    let ruleflow = event.content[0];
    let title = showEmoji ? emoji(event) : "";
    const ruleflowLink = (event.details && event.details[0] && event.details[0].targetURL) ? event.details[0].targetURL : event.sourceLink;
    title += "  Ruleflow <" + ruleflowLink + "|" + ruleflow.name + ">";
    if (type === EVENT_RULEFLOW_CREATED) {
        title += " was created";
    } else if (type === EVENT_RULEFLOW_UPDATED) {
        if(event.details[0].updateType ==="RENAME") {
            title += " was renamed from '" + event.details[0].oldValue + "'";
        } else {
            title += " was updated";
        }
    } else if (type === EVENT_RULEFLOW_DELETED) {
        title += " was deleted";
    }
