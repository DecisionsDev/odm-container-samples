...
const EVENT_OPERATION_CREATED = "OperationCreated";
const EVENT_OPERATION_UPDATED = "OperationUpdated";
const EVENT_OPERATION_DELETED = "OperationDeleted";
...
const slackPretext = (event, showEmoji) => {
    ...
    let operation = event.content[0];
    let title = showEmoji ? emoji(event) : "";
    const operationLink = (event.details && event.details[0] && event.details[0].targetURL) ? event.details[0].targetURL : event.sourceLink;
    title += "  Operation <" + operationLink + "|" + operation.name + ">";
    if (type === EVENT_OPERATION_CREATED) {
        title += " was created";
    } else if (type === EVENT_OPERATION_UPDATED) {
        if(event.details[0].updateType ==="RENAME") {
            title += " was renamed from '" + event.details[0].oldValue + "'";
        } else {
            title += " was updated";
        }
    } else if (type === EVENT_OPERATION_DELETED) {
        title += " was deleted";
    }
