...
const EVENT_RULEPACKAGE_CREATED = "RulePackageCreated";
const EVENT_RULEPACKAGE_UPDATED = "RulePackageUpdated";
const EVENT_RULEPACKAGE_DELETED = "RulePackageDeleted";
...
const slackPretext = (event, showEmoji) => {
    ...
    const packageLink = (event.details && event.details[0] && event.details[0].targetURL) ? event.details[0].targetURL : event.sourceLink;
    title += "  RulePackage <" + packageLink + "|" + testsuite.name + ">";
    if (type === EVENT_RULEPACKAGE_CREATED) {
        title += " was created";
    } else if (type === EVENT_RULEPACKAGE_UPDATED) {
        if(event.details[0].updateType ==="RENAME") {
            title += " was renamed from '" + event.details[0].oldValue + "'";
        } else {
            title += " was updated";
        }    } else if (type === EVENT_RULEPACKAGE_DELETED) {
        title += " was deleted";
    }
