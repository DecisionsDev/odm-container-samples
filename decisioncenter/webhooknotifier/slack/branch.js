...
const EVENT_BRANCH_CREATED = "BranchCreated";
const EVENT_BRANCH_UPDATED = "BranchUpdated";
const EVENT_BRANCH_DELETED = "BranchDeleted";
...
const slackPretext = (event, showEmoji) => {
    ...
    let branch = event.content[0];
    let title = showEmoji ? emoji(event) : "";
    const branchLink = (event.details && event.details[0] && event.details[0].targetURL) ? event.details[0].targetURL : event.sourceLink;
    title += "  Branch <" + branchLink + "|" + branch.name + ">";
    if (type === EVENT_BRANCH_CREATED) {
        title += " was created";
    } else if (type === EVENT_BRANCH_UPDATED) {
        if(event.details[0].updateType ==="RENAME") {
            title += " was renamed from '" + event.details[0].oldValue + "'";
        } else {
        title += " was updated";
        }
    } else if (type === EVENT_BRANCH_DELETED) {
        title += " was deleted";
    }
