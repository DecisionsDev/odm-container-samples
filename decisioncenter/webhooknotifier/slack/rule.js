...
const EVENT_RULE_CREATED = "RuleCreated";
const EVENT_RULE_UPDATED = "RuleUpdated";
const EVENT_RULE_DELETED = "RuleDeleted";
...
const slackPretext = (event, showEmoji) => {
    ...
    let ruleArtifact = event.content[0];
    let title = showEmoji ? emoji(event) : "";
    const artifactLink = (event.details && event.details[0] && event.details[0].targetURL) ? event.details[0].targetURL : event.sourceLink;
    title += "  Rule artifact <" + artifactLink + "|" + ruleArtifact.name + ">";
    if (type === EVENT_RULE_CREATED) {
        title += " was created";
    } else if (type === EVENT_RULE_UPDATED) {
        if(event.details[0].updateType ==="RENAME") {
            title += " was renamed from '" + event.details[0].oldValue + "'";
        } else {
            title += " was updated";
        }
    } else if (type === EVENT_RULE_DELETED) {
        title += " was deleted";
    }
